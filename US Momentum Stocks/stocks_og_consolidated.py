...
class StockOnlyMomentum(QCAlgorithm):
    ...
    # Shared golden-ratio-derived fractions used to build both the
    # universe-breadth bands (OnData) and the per-symbol ceiling bands
    # (Rebalance). Listed largest -> smallest; _build_bands mirrors them
    # around `mid` to produce the full symmetric band ladder.
    BREADTH_BAND_FRACTIONS = [1.618, 1.382, 1.0, 0.809, 0.5, 0.382]
    ...
    def OnData(self, data):
        ...
        for s in list(self.symbols):
            ...
            # 3. Calculate the Price Bands (fixed-unit breadth bands)
            bands = self._build_bands(mid, dev)

            # 4. Map price to Band Index
            idx = self._band_index(close, bands)
            self.current_band_idx[s] = idx

    def _build_bands(self, mid, dev, unit=1.0, fractions=None):
        """
        Build a symmetric ladder of price bands around `mid`.

        Shared by the breadth-tracking bands in OnData (unit=1.0, default
        fractions) and the historical-ceiling bands in Rebalance (unit
        scaled by the stretch EMA, custom fractions), so the two callers
        stay in sync instead of maintaining two hand-written band lists.

        Parameters
        ----------
        mid : float
            Center of the bands (e.g., EMA).
        dev : float
            Standard deviation used to scale band width.
        unit : float, optional
            Extra multiplier applied to every fraction before scaling by
            `dev` (e.g., a stretch-EMA value). Defaults to 1.0.
        fractions : list[float], optional
            Ordered fractions, largest to smallest, used to build the
            lower half of the ladder; the upper half mirrors them.
            Defaults to BREADTH_BAND_FRACTIONS.

        Returns
        -------
        list[float]
            Band boundaries, ascending, with `mid` at the center.
        """
        fracs = fractions if fractions is not None else self.BREADTH_BAND_FRACTIONS
        lower = [mid - dev * unit * f for f in fracs]
        upper = [mid + dev * unit * f for f in reversed(fracs)]
    ...
    def Rebalance(self):
        ...
        # Historical-ceiling bands are built from the same golden-ratio
        # fractions as the breadth bands, but scaled by each symbol's own
        # stretch EMA (lm) instead of a fixed unit of 1.0.
        ...
        for s in top:
            ...
            bands = self._build_bands(mid, dev, unit=lm, fractions=ceiling_fractions)
            ...
            # Anticipatory Exhaustion Scaling: pull the current stretch and
            # the peak stretch we recorded in OnData. If we're in high
            # bands (idx >= 10) but the stretch has decayed 20% from peak,
            # override the scale to its minimum (0.2) because the momentum
            # is 'exhausted' even if the price is still high.
